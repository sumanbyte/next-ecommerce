...
export default async function PATCH(req: NextApiRequest, res:NextApiResponse) {
    ...
    if(!productId){
        return res.status(400).json({message: "A Valid productId id is required"});
    }
    ...
    if(typeof quantity !== "string"){
        return res.status(400).json({message: "quantity must be type of string."})
    }
    ...
    const dbCart = await Cart.findOne({productId});
    ...
    const data = await Cart.updateOne({_id: dbCart._id}, {quantity: currentQuantity});
